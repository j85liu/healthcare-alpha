...
ClinicalTrials.gov trial events and FDA approvals arrive on their own
native calendars (posting/completion dates, decision dates), not on each
...
def align_to_fiscal_quarters(raw_series: list[dict], fiscal_periods: list[dict]) -> list[dict]:
    ...
    Args:
        raw_series: alt-data observations (e.g. trial phase-completion
            events, or FDA approval/decision events).
    ...
    TODO:
        - Decide aggregation per signal type (count of trial events in
          period, count of approvals in period, etc).
        - Handle fiscal quarters that don't align to calendar quarters.
        - Trial-to-revenue lag is much longer and noisier than a device
          clearance-to-revenue lag (launch timing, patent life, payer
          coverage decisions) — don't assume the same lag structure as
          medtech when this gets implemented.
    """
